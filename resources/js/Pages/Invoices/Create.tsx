...
import { Client, InvoiceStatus, PageProps } from '@/types';
...
export default function Create({
    clients,
    preselect,
}: PageProps<{ clients: ClientOption[]; preselect: number | null }>) {
    const initialClient = preselect ? (clients.find((c) => c.id === preselect) ?? null) : null;

    const [search, setSearch] = useState(initialClient ? displayName(initialClient) : '');
    const [open, setOpen] = useState(false);
    const [selectedClient, setSelectedClient] = useState<ClientOption | null>(initialClient);
    const wrapperRef = useRef<HTMLDivElement>(null);

    const { data, setData, post, processing, errors } = useForm({
        client_id: preselect ? String(preselect) : '',
        service_date: '',
        amount_due: '',
        notes: '',
    ...
    function selectClient(c: ClientOption) {
        ...
        const balance = Number(c.patient_balance) > 0
            ? Number(c.patient_balance)
        ...
        setData((prev) => ({
            ...
            // Only pre-fill if amount_due is still empty
            amount_due: prev.amount_due || (balance ? balance.toFixed(2) : ''),
        }));
    }
    ...
    const filtered = (() => {
        ...
        if (!search || selectedClient) return clients.slice(0, 50);
        return clients
            .filter((c) => {
        ...
        <AuthenticatedLayout
            header={
                <div className="flex items-center gap-3">
                    ...
                    <h2 className="text-xl font-semibold leading-tight text-slate-900">New Invoice</h2>
                </div>
            }
        >
                ...
                <div className="mx-auto max-w-2xl space-y-5 sm:px-6 lg:px-8">
                    ...
                    <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                        ...
                        <div ref={wrapperRef} className="relative">
                            <InputLabel htmlFor="client_search" value="Search Patient *" />
                            <div className="relative mt-1">
                                ...
                                <input
                                    ...
                                    type="text"
                                    autoComplete="off"
                                    className="block w-full rounded-md border-slate-300 bg-white py-2 pl-9 pr-10 text-sm shadow-sm focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500"
                                    placeholder="Type name, email, or phone…"
                                    value={search}
                                    onFocus={() => { if (!selectedClient) setOpen(true); }}
                                    onChange={(e) => {
                                ...
                                <ul className="absolute z-20 mt-1 max-h-64 w-full overflow-y-auto rounded-md border border-slate-200 bg-white shadow-lg">
                                    {filtered.length === 0 ? (
                                        ...
                                        filtered.map((c) => (
                                            <li
                                                key={c.id}
                                                className="cursor-pointer border-b border-slate-50 px-4 py-2.5 last:border-0 hover:bg-brand-50"
